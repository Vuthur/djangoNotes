...
- 说明:
    - protocol（协议）(http://)
        - **http** 通过 HTTP 访问该资源。 格式 `HTTP://`     
          目前使用最多(明文协议, 明文传输可见  早期展示型/ 宣传性网站,  无需加密)
          特点: 无需加解密, 传输速度较快
        - **https** 通过安全的 HTTPS 访问该资源。 格式 `HTTPS://`    
  (密文传输  适用于更多的业务,  特别是由资金往来的业务)
          特点: 更加安全, 传输速度相对http较慢
    ...
    - port（端口号）
    - 整数，**可选**(即可写可不写)，省略时使用方案的默认端口；
        - 各种传输协议都有默认的端口号，如http的默认端口为**80**。
    - path（路由地址）(/video/showVideo)
        - 由零或多个“/”符号隔开的字符串，一般用来表示主机上的一个目录或文件地址。路由地址决定了服务器端如何处理这个请求
    
    - query(查询/ 查询字符串)  (?menuId=657421&version=AID201908)
        - 可选，用于给动态网页传递参数，可有多个参数，用“&”符号隔开，每个参数的名和值用“=”符号隔开。
        - 由客户端浏览器向服务器传递参数的一种方式
    - fragment（信息片断）(**锚点**:  定位使用 , 可以定位到页面中的指定位置)
